...
class EcgGlove:
    ...
    def __init__(
        ...
        self.hp_filter_type = hp_filter_type  # Store the filter type

        # Initialize optimized filters
        self.optimized_filters = OptimizedECGFilters(sampling_rate)

        # Initialize vectorized processor with better error handling
        self.vectorized_processor = None
        try:
            logger.info("Attempting to initialize vectorized processor...")
            from ecg_filters import VectorizedECGProcessor

            # Check if the class exists and has required methods
            if hasattr(VectorizedECGProcessor, "process_leads_batch"):
                self.vectorized_processor = VectorizedECGProcessor(self.sampling_rate)
                logger.info("Vectorized processor initialized successfully")
            else:
                logger.warning("VectorizedECGProcessor missing required methods")

        except ImportError as e:
            logger.warning(f"Could not import VectorizedECGProcessor: {e}")
            logger.info("Vectorized processing will not be available")
        except Exception as e:
            logger.error(f"Failed to initialize vectorized processor: {e}")
            logger.info("Falling back to standard processing methods")

        self._filter_cache = {}  # Cache for filter coefficients
        ...
        # Initialize decoder for process_optimized method
    ...
    def decode_data(self, data_bytes: bytes) -> None:
        ...
        for lead, signal_data in self.raw_signals.items():
            ...
            filtered = self._filter_signal_optimized(signal_data)
    ...
    def process_optimized(self) -> Dict[str, Any]:
        """Memory-efficient ECG processing"""

        # Get raw leads data
        raw_leads = self.decoder.get_leads()

        # Process in chunks if data is large
        chunk_size = 10000  # Process in chunks of 10k samples

        processed_leads = {}
        for lead_name, signal in raw_leads.items():
            if len(signal) > chunk_size:
                # Process large signals in chunks
                chunks = []
                for i in range(0, len(signal), chunk_size):
                    chunk = signal[i : i + chunk_size]
                    filtered_chunk = self._filter_signal_optimized(chunk)
                    chunks.append(filtered_chunk)
                processed_leads[lead_name] = np.concatenate(chunks)
            else:
                processed_leads[lead_name] = self._filter_signal_optimized(signal)

        # Use the existing quality processor
        from ecg_processor import LeadData

        lead_data: LeadData = {
            "lead_signals": raw_leads,
            "cleaned_signals": processed_leads,
        }

        quality_results = self.quality_processor.analyze_all_leads(lead_data)

        return {
            "quality": quality_results,
            "measurements": self._calculate_measurements_if_needed(),
        }
    ...
    def _filter_signal_optimized(self, raw: np.ndarray) -> np.ndarray:
        """Optimized signal filtering using vectorized operations"""
        filtered = raw.copy()
        ...
        # High-pass filter (vectorized)
        if self.hp_filter_type:
            cache_key = f"hp_{self.hp_filter_type.value}"
            if cache_key not in self._filter_cache:
                if self.hp_filter_type == HPFilterType.HP005:
                    cutoff = 0.5
                else:  # HP015
                    cutoff = 1.5
                self._filter_cache[cache_key] = butter(
                    1, cutoff, btype="highpass", fs=self.sampling_rate, output="sos"
                )

            filtered = sosfilt(self._filter_cache[cache_key], filtered)
            # Ensure we have a numpy array
            if isinstance(filtered, tuple):
                filtered = filtered[0]  # Take first element if it's a tuple
            filtered = np.asarray(filtered, dtype=np.float64)

        # Notch filters (batch application)
        if self.filters:
            for freq in self.filters:
                cache_key = f"notch_{freq}"
                if cache_key not in self._filter_cache:
                    Q = 30
        ...
        # Optimized morphology filter
        if self.spike_removal:
            filtered = fast_morphology_filter(filtered)
    ...
    def _calculate_measurements_if_needed(self) -> Dict[str, Any]:
        """
        Calculate ECG measurements if needed.
        This is a placeholder for additional measurement calculations.
        """
        # For now, return empty measurements dict
        # This could be extended to include heart rate, intervals, etc.
        return {
            "heart_rate_bpm": None,
            "pr_interval_ms": None,
    ...
    def process_leads_parallel(
        ...
        """
        Process ECG leads in parallel for improved performance
        """

        def process_single_lead(args):
            lead_name, signal = args
            return lead_name, self._filter_signal_optimized(signal)

        # Use ProcessPoolExecutor for CPU-bound filtering tasks
        with ProcessPoolExecutor(max_workers=min(4, len(leads_data))) as executor:
            # Submit all lead processing tasks
            futures = {
                executor.submit(process_single_lead, (lead_name, signal)): lead_name
            ...
            for future in as_completed(futures):
                try:
                    lead_name, filtered_signal = future.result()
                    processed_leads[lead_name] = filtered_signal
                except Exception as e:
                    lead_name = futures[future]
                    print(f"Error processing lead {lead_name}: {e}")
                    # Fallback to original signal
                    processed_leads[lead_name] = leads_data[lead_name]

        return processed_leads

    def process_with_vectorization(self) -> Dict[str, Any]:
        """
        Use vectorized processing for better performance with proper error handling
        """
        try:
            if not self.raw_signals:
                raise RuntimeError("No raw signals available. Call decode_data first.")

            logger.info(
                f"Starting vectorized processing with {len(self.raw_signals)} signals"
            )

            # Check if vectorized processor is available
            if self.vectorized_processor is None:
                logger.warning(
                    "Vectorized processor not available, falling back to optimized processing"
                )
            ...
            config = {
                ...
                "hp_filter_type": (
                    self.hp_filter_type.value if self.hp_filter_type else "HP015"
                ),
                "enable_baseline_correction": self.enable_baseline_correction,
                "enable_smoothing": self.enable_smoothing,
            }

            logger.info(f"Vectorized config: {config}")

            # Check if the processor has the required method
            if not hasattr(self.vectorized_processor, "process_leads_batch"):
                logger.error(
                    "Vectorized processor missing 'process_leads_batch' method"
                )
                raise AttributeError("Vectorized processor missing required method")

            # Use vectorized processor for batch processing
            logger.info("Calling vectorized processor...")
            processed_leads = self.vectorized_processor.process_leads_batch(
                self.raw_signals, config
            )

            if not processed_leads:
                raise RuntimeError("Vectorized processing returned empty results")

            logger.info(f"Processed {len(processed_leads)} leads successfully")

            # Update internal state
            self.lead_signals = processed_leads
            self.cleaned_signals = processed_leads.copy()  # For now, treat as cleaned
            ...
            logger.error(f"Vectorized processing failed: {str(e)}", exc_info=True)
            logger.info("Falling back to optimized processing")
            # Fallback to optimized processing
            return self.process_optimized()

    def is_vectorized_available(self) -> bool:
        """Check if vectorized processing is available"""
        return self.vectorized_processor is not None and hasattr(
            self.vectorized_processor, "process_leads_batch"
        )

    # Add a method to get processing capabilities
    def get_processing_capabilities(self) -> Dict[str, bool]:
        """Get available processing methods"""
        return {
            "vectorized": self.is_vectorized_available(),
