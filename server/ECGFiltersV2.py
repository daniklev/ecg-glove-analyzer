...
class OptimizedECGFilters:
    """Vectorized and optimized ECG filter implementations"""
    ...
    def _initialize_filters(self):
        ...
        self.hp_filters = {
            "HP005": butter(
                1, 0.5, btype="highpass", fs=self.sampling_rate, output="sos"
            ),
            "HP015": butter(
                1, 1.5, btype="highpass", fs=self.sampling_rate, output="sos"
            ),
            "HP05": butter(
                1, 5.0, btype="highpass", fs=self.sampling_rate, output="sos"
            ),
        }
        ...
        for freq in [50, 60, 100, 120]:
            ...
            self.notch_filters[freq] = butter(
                2,
                [w0 - w0 / Q, w0 + w0 / Q],
                btype="bandstop",
...
@jit(nopython=True)
def fast_morphology_filter(data: np.ndarray) -> np.ndarray:
    """Optimized morphology filter using Numba JIT compilation"""
    filtered = np.zeros_like(data)

    # Simplified morphology operation
    for i in range(2, len(data) - 2):
        # 5-point median-like operation
        window = data[i - 2 : i + 3]
...
class VectorizedECGProcessor:
    """Vectorized ECG processing for better performance"""

    def __init__(self, sampling_rate: int = 500):
        self.sampling_rate = sampling_rate
        self.filters = OptimizedECGFilters(sampling_rate)

    def process_leads_batch(
        self, leads_data: Dict[str, np.ndarray], config: Dict[str, Any]
    ) -> Dict[str, np.ndarray]:
        """Process all leads simultaneously using vectorization"""

        # Stack all lead signals for batch processing
        lead_names = list(leads_data.keys())
        signals_matrix = np.stack([leads_data[name] for name in lead_names], axis=0)

        # Apply filters in batch
        if config.get("spike_removal", False):
            # Apply morphology filter to all leads
            signals_matrix = fast_morphology_filter(signals_matrix)
            # for i in range(signals_matrix.shape[0]):
            #     signals_matrix[i] = fast_morphology_filter(signals_matrix[i])

        # Apply frequency domain filters
        if config.get("filters", []):
            for freq in config["filters"]:
                if freq in self.filters.notch_filters:
                    signals_matrix = sosfilt(
                        self.filters.notch_filters[freq], signals_matrix, axis=1
                    )

        # High-pass filter
        hp_type = config.get("hp_filter_type", "HP015")
        if hp_type in self.filters.hp_filters:
            signals_matrix = sosfilt(
                self.filters.hp_filters[hp_type], signals_matrix, axis=1
            )

        # Convert back to dictionary
        return {name: signals_matrix[i] for i, name in enumerate(lead_names)}
